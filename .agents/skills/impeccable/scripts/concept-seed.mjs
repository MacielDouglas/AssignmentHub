...
export function selectApprovedStagings({
	scope,
	key,
	reroll = 0,
	mode = null,
...
export function selectApprovedChallengers({
	...
	const ticketsFor = (pool) =>
		pool.flatMap((concept) => {
	...
	if (_resolvedData === undefined) {
		...
		if (local) {
			...
			data = {
				...
				catalogCount,
				challengers: picks,
				stagings: selectApprovedStagings({
					scope,
					key,
					reroll,
					mode,
			...
			return fetchRoll({ scope, key, mode, reroll }).then((roll) =>
				renderConceptSeed({
					scope,
					key,
					reroll,
					...
					_resolvedData: roll
						? {
								...
								stagings: Array.isArray(roll.stagings)
									? roll.stagings
									: roll.staging
										? [roll.staging]
										: [],
