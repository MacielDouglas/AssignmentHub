...
(() => {
	...
	const ANTIPATTERNS = [
		...
		},
		{
			...
			skillSection: "Typography",
			skillGuideline:
				"repeated eyebrow or kicker labels as section scaffolding",
		},
		{
	...
	function scanCssTextForGlow(content) {
		...
		while ((m = shadowRe.exec(content)) !== null) {
			...
			for (const layer of value.split(/,(?![^(]*\))/)) {
				const colorInfo = findShadowColor(layer);
				if (!colorInfo?.color || !hasChroma(colorInfo.color, 30)) continue;
				const vals = extractShadowLengths(
					layer,
					colorInfo.start,
					colorInfo.end,
	...
	function _buildCustomPropMap(document) {
		const map = new Map();
		let sheets;
		try {
	...
	function collectRepeatedSectionKickerCandidates(
		doc,
		getStyle,
	...
	function _checkNumberedSectionLabelsFromDoc(doc, win) {
	...
	function _checkElementRadialSpotlight(el, style, _tag, _window) {
		const gradientValue = elementGradientValue(style, el);
		if (!gradientValue) return [];
	...
	function isVisuallyHidden(el, style) {
		if (el.matches?.(SR_ONLY_SELECTOR) || el.closest?.(SR_ONLY_SELECTOR))
			return true;
		...
		// Uppercase letterspaced micro-labels are still functional — not exempt.
		{
			...
			if (
				...
				const isExemptContext =
					el.matches?.(EXEMPT_CONTEXT) || el.closest?.(EXEMPT_CONTEXT);
				if (!isExemptContext && !isVisuallyHidden(el, style)) {
					...
					const isInteractive =
						el.matches?.(INTERACTIVE) || el.closest?.(INTERACTIVE);
					const isFurniture =
						el.matches?.(FURNITURE) || el.closest?.(FURNITURE);
					const isSmallprint =
						el.matches?.(SMALLPRINT) || el.closest?.(SMALLPRINT);
	...
	function _checkElementQuality(el, style, tag, window) {
		const hasDirectText = [...el.childNodes].some(
			(n) => n.nodeType === 3 && n.textContent.trim().length > 10,
		);
		const textLen = el.textContent?.trim().length || 0;
	...
	function _checkElementHoverContrast(el, style, tag, window) {
	...
	function _checkElementIconTile(el, tag, window) {
		if (!HEADING_TAGS.has(tag)) return [];
		const sibling = el.previousElementSibling;
		if (!sibling) return [];
	...
	function _checkElementItalicSerif(el, style, tag) {
		if (tag !== "h1" && tag !== "h2") return [];
		return checkItalicSerif({
			tag,
			fontStyle: style.fontStyle || "",
	...
	function _checkElementHeroEyebrow(el, style, tag, window, customPropMap) {
		if (tag !== "h1") return [];
		const sibling = el.previousElementSibling;
		if (!sibling) return [];
		const sibStyle = window.getComputedStyle(sibling);
	...
	function _checkRepeatedSectionKickersFromDoc(doc, win) {
		const candidates = collectRepeatedSectionKickerCandidates(
			doc,
			(el) => win.getComputedStyle(el),
			(value, fontSize) => resolveLengthPx(value, fontSize) || 0,
	...
	function _checkElementMotion(tag, style) {
		return checkMotion({
			tag,
			transitionProperty: style.transitionProperty || "",
			animationName: style.animationName || "",
	...
	function _checkElementGlow(tag, style, effectiveBg) {
		const boxShadow =
			style.boxShadow && style.boxShadow !== "none" ? style.boxShadow : "";
		const textShadow =
			style.textShadow && style.textShadow !== "none" ? style.textShadow : "";
	...
	function _checkPageTypography(doc, win) {
		const findings = [];
	...
	function _checkPageLayout(doc, win) {
		const findings = [];
	...
	function _checkRepeatedContainerTextFromDoc(doc, win) {
	...
	function _checkElementOversizedH1(el, _style, tag, window) {
		if (tag !== "h1") return [];
		const fontSize = resolveFontSizePx(el, window);
	...
	function _checkElementGptBorderShadow(_el, style) {
	...
	function _checkElementClippedOverflow(el, style, _tag, window) {
	...
	function _checkContentHiddenAtRest({
		totalChars = 0,
		hiddenChars = 0,
		hiddenSamples = [],
	} = {}) {
