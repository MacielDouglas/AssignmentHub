...
function isAccentColor(cssColor) {
	...
	const _rgbM = /rgba?\(\s*(\d+)\s*,?\s+|\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/.exec(
...
function checkElementRadialSpotlight(el, style, _tag, _window) {
	const gradientValue = elementGradientValue(style, el);
	if (!gradientValue) return [];
	...
	// Uppercase letterspaced micro-labels are still functional — not exempt.
	{
		...
		if (
			...
			if (!isExemptContext && !isVisuallyHidden(el, style)) {
				...
				const isFurniture = el.matches?.(FURNITURE) || el.closest?.(FURNITURE);
				const isSmallprint =
...
function measureHiddenTextDOM() {
	const cache = new Map();
	function stateOf(el) {
		if (el?.nodeType !== 1 || el === document.documentElement) return "visible";
