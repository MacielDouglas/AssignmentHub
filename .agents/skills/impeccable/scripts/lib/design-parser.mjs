...
const _RGBA_RE = /rgba?\([^)]+\)/gi;
const _BOX_SHADOW_RE = /(?:box-shadow:\s*)?((?:-?\d[\w\d\s\-.,/()#%]*)+)/;
const _NAMED_RULE_RE = /\*\*(The [^*]+?Rule)\.\*\*\s*(.+)/;
...
function _scanInlineColors(lines) {
	const out = [];
...
function _parseStitchInlineGroups(lines) {
