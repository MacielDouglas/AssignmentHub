...
(() => {
	...
	function _configureModifierPillStyle(extra = {}) {
		const P = configureBarPalette();
		return {
			display: "inline-flex",
			alignItems: "center",
	...
	function _bindConfigureModifierPillHover(btn, controlsLocked) {
		btn.addEventListener("mouseenter", () => {
			if (controlsLocked) return;
			const P = configureBarPalette();
			btn.style.color = P.text;
	...
	function generationStatusText() {
		...
		if (generationPhase === "first_variant_generating")
			return "Designing the first variant...";
		if (generationPhase === "first_variant_validating")
			return "Checking the first variant...";
		if (generationPhase === "remaining_variants_generating")
			return "Exploring two more directions...";
		if (generationPhase === "remaining_variants_validating")
	...
	function wrapMixedContentTextNodes(rootEl) {
		...
		for (const child of Array.from(rootEl.children)) {
			if (!child.dataset?.impeccableTextWrap) {
	...
	function enterEditingMode() {
		...
		if (inlineEditRows.length > 0) {
			const firstEditable = inlineEditRows[0]?.el;
			setTimeout(() => {
				const el = firstEditable;
				if (!el?.isConnected || state !== "EDITING") return;
				el.focus();
	...
	function mixedTextWrapRestoreHint(el) {
		if (el.dataset?.impeccableTextWrap !== "true" || !el.parentElement)
			return null;
	...
	function resolveComponentModuleUrl(_manifest, modulePath) {
	...
	function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
		...
		scrollLockTargetY =
			typeof initialTargetY === "number" && Number.isFinite(initialTargetY)
				...
				: window.scrollY;
		scrollLockAnchorTop =
			typeof initialAnchorTop === "number" && Number.isFinite(initialAnchorTop)
				? initialAnchorTop
		...
		const correct = (_why) => {
			...
			if (
				...
				Number.isFinite(scrollLockAnchorTop)
			) {
		...
		const reanchor = (_why) => {
			...
			const _prevTarget = scrollLockTargetY;
			scrollLockTargetY = window.scrollY;
			...
			switch (msg.type) {
				...
				case "agent_phase":
					if (
						...
						if (
							msg.phase === "variant_parameters_generating" ||
							msg.phase === "variant_parameters_validating"
						) {
							parameterGenerationState = "loading";
						}
						if (
							msg.phase === "parameters_ready" &&
							parameterGenerationState !== "ready"
						) {
							parameterGenerationState = "loading";
						}
...
void main() {
  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
	...
	function _commitAcceptedVariantToDom(sessionId, variantId) {
		const wrapper = document.querySelector(
			`[data-impeccable-variants="${sessionId}"]`,
		);
		if (!wrapper) return false;
		...
		if (!accepted?.firstElementChild) return false;
		const parent = wrapper.parentElement;
		if (!parent) return false;
	...
	function _loadPickPref() {
		return loadInteractionPrefs().pickActive;
	}

	function _savePickPref() {
	...
	const _PAGE_CHAT_PROCESSING_W = "76px";
