...
Output (JSON):
  { ok, file, inserted|removed, config? }`);
	...
	const tanstack =
		svelteKit || nuxt ? null : detectTanStackStartProject(process.cwd());

	if (args.includes("--remove")) {
		if (svelteKit) {
			const adapterResult = removeSvelteKitLiveAdapter({
				cwd: process.cwd(),
				config,
			});
			console.log(
		...
		if (tanstack) {
			const adapterResult = removeTanStackLiveAdapter({
				cwd: process.cwd(),
				project: tanstack,
			});
			console.log(
	...
	if (tanstack) {
		const adapterResult = applyTanStackLiveAdapter({
			cwd: process.cwd(),
			port,
			token,
			project: tanstack,
