...
`);
	...
	function Se(e, t) {
		...
		try {
			return (
				n && i?.putImageData(n.getImageData(0, 0, e.width, e.height), 0, 0),
				r
			);
		} catch (a) {
			t.log.warn("Failed to clone canvas", a);
  ...
  `);
	...
	function z(e, t) {
		...
		if (!E) {
			m &&
				...
				m.test(s) &&
				(s += (/\?/.test(s) ? "&" : "?") + Date.now());
			const b = n.startsWith("font") && h?.minify,
				w = new Set();
			b &&
				n
					.split(";")[1]
	...
	function $t(e, t) {
		const r = [];
		return (
			e.replace(ke, (n, _i, a) => (r.push([a, ue(a, t)]), n)),
			r.filter(([n]) => !re(n))
		);
	}
	function Lt(e) {
	...
	function jt(e, t) {
		...
		if (s) {
			...
			if ((a && e.setAttribute("href", c), n?.querySelector(c)))
				return [];
