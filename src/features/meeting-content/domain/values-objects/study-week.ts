...
export function parseStudyWeekLabel(
	 ...
	 */
	const crossMonth =
		/(\d{1,2})\s*(?:de\s+)?([a-záéíóúüñç]+)\s*-\s*(\d{1,2})\s*(?:de\s+)?([a-záéíóúüñç]+)(?:\s+de)?(?:\s+20\d{2})?/iu.exec(
			normalized,
		);
	...
	if (crossMonth) {
		...
		try {
			...
			const endDiff = Math.floor(
				(statedEnd.getTime() - weekStart.getTime()) / (1000 * 60 * 60 * 24),
			);
	 ...
	 */
	const sameMonth =
		/(\d{1,2})\s*(?:-|a)\s*(\d{1,2})\s*(?:de\s+)?([a-záéíóúüñç]+)(?:\s+de)?(?:\s+20\d{2})?/iu.exec(
			normalized,
		);
