import type { WeekdayMeetingPdfData } from "./weekday-meeting-pdf-types";
...
export function scalePdfLayout(spacing: number, font: number): PdfLayout {
	const scaleSpacing = (value: number) =>
		Math.round(value * spacing * 100) / 100;
...
export type PdfPageLayout<T = WeekdayMeetingPdfData> = {
	topMeeting: T | null;
	bottomMeeting: T | null;
};
...
export function planPages<T>(meetings: T[]): PdfPageLayout<T>[] {
